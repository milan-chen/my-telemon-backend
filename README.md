...
├── config.py                   # 配置文件读取和验证模块
├── app_config.yaml.template    # 配置文件模板（版本控制）
├── app_config.yaml             # 实际配置文件（本地，已忽略）
├── requirements.txt            # Python 依赖列表
├── start.sh                    # 智能启动脚本（支持配置管理）
...
项目使用 `app_config.yaml` 作为配置文件，包含所有必需的 API 凭证和服务器设置。
...
默认会话文件存储在 `sessions/` 目录。可以通过修改配置文件中的 `session_dir` 来自定义：

```yaml
server:
  host: "0.0.0.0"
  port: 8080
  session_dir: "custom_sessions"  # 自定义会话目录
```
...
1. **配置文件缺失**
   ...
   - 或手动复制模板：`cp app_config.yaml.template app_config.yaml`
...
1. **配置文件安全**: 
   - `app_config.yaml` 包含敏感信息，已在 `.gitignore` 中排除，不会被提交到版本控制
   - 仅 `app_config.yaml.template` 模板文件纳入版本控制
   - 配置重置功能会自动备份现有配置，避免意外丢失
